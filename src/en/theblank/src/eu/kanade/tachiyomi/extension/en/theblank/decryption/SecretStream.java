...
public class SecretStream {
    ...
    public PullResult pull(State state, byte[] in, int inlen, byte[] ad, int adlen) {
        if (inlen < ABYTES) {
            android.util.Log.e("SecretStream", "Input too short: " + inlen + " < " + ABYTES);
        ...
        android.util.Log.d("SecretStream", "=== Decryption Start ===");
        android.util.Log.d("SecretStream", "Input length: " + inlen);
        android.util.Log.d("SecretStream", "Expected message length: " + mlen);
        android.util.Log.d("SecretStream", "Nonce: " + bytesToHex(state.nonce));
        android.util.Log.d("SecretStream", "Key: " + bytesToHex(state.k));
        ...
        // Process the tag byte (encrypted)
        ...
        android.util.Log.d("SecretStream", "Decrypted message (first 32 bytes): " + bytesToHex(Arrays.copyOf(m, Math.min(32, m.length))));

        // XOR inonce with MAC
        for (int i = 0; i < 8; i++) {
            state.nonce[4 + i] ^= mac[i];
        }
        android.util.Log.d("SecretStream", "Updated nonce: " + bytesToHex(state.nonce));
        ...
        android.util.Log.d("SecretStream", "=== Decryption Success ===");
